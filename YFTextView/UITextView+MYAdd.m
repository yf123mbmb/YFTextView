...
- (void)swizzling_layoutSubviews {
    ...
    CGFloat lineHeight = 0;//self.font ? self.font.lineHeight : 15.f;
    UIEdgeInsets edgeInsets = self.textContainerInset;
    self.placeholderLabel.textAlignment = self.textAlignment;
    
    self.placeholderLabel.frame = CGRectMake(edgeInsets.left+5, edgeInsets.top, CGRectGetWidth(self.bounds)-(edgeInsets.left)*2-5, lineHeight);
    
    [self.placeholderLabel sizeToFit];
    self.placeholderLabel.frame = CGRectMake(self.placeholderLabel.frame.origin.x, self.placeholderLabel.frame.origin.y, self.placeholderLabel.frame.size.width, self.placeholderLabel.frame.size.height);
